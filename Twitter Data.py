...
# nopunc = [c for c in df1.text if c not in string.punctuation]
...
# tweet = html_parser.unescape(list_text)
# print(tweet)
...
# for i in range(len(list_text[:5])):
#     print('Example #{0:d}: "{1:s}"'.format(i+1,list_text[:5][i]))
...
# for i in range(len(list_text[:50])):
#     print('Cleaned example #{0:d}: "{1:s}"'.format(i+1,corpus_clean[:50][i]))
...
# import re
# def clean_html(text_values):
#     cleanr = re.compile('<.*?>@;')
#     cleantext = re.sub(cleanr, ' ', text_values)
#     return cleantext
# # def clean_punc(word):
# #     cleaned = re.sub(r'[?|!|\'|#]', r'', word)
# #     cleaned = re.sub(r'[.|,|)|(|\|/]', r' ', cleaned)
...
#  tweet = original_tweet.decode("utf8").encode(‘asc11’,'ignore’)
